...
interface FindingShowLayoutProps {
    finding: {
        data: Finding;
    };
    type: 'AUD' | 'ABN';
...
export default function FindingShowLayout({ finding, type }: FindingShowLayoutProps) {
    const strings = UI_STRINGS;
    const code = finding?.data?.type?.code;

    const breadcrumbs: BreadcrumbItem[] = [
        {
            title: code == 'ABN' ? (strings.ABNORMALITY?.plural ?? 'Abnormalities') : (strings.AUDIT?.plural ?? 'Audits'),
            href: code == 'ABN' ? route('abnormalities.index') : route('audits.index'),
        },
        {
            title: finding.data?.equipment?.code ?? truncateText(finding.data.functionalLocation?.description ?? '', 25),
            href: code == 'ABN' ? route('abnormalities.index', finding.data.id) : route('audits.index', finding.data.id),
        },
    ];
