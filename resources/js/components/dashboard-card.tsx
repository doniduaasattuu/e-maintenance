...
export default function DashboardCard({ value, description, title, children }: DashboardCardProps) {
    return (
        <Card className="bg-sidebar flex h-full flex-col space-y-3 lg:space-y-5 xl:space-y-6">
            <CardHeader className="mb-2 flex flex-row items-center justify-between space-y-0 md:pb-2">
                <CardTitle className="text-muted-foreground text-sm font-medium">{title}</CardTitle>
                <div className="shrink-0">{children}</div>
            </CardHeader>
            <CardContent className="space-y-2 lg:space-y-3 xl:space-y-4">
                <div className="text-3xl font-bold sm:text-4xl md:text-5xl">{value}</div>
